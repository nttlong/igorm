...
func (m *MigratorLoaderPostgres) LoadForeignKey(db *tenantDB.TenantDB) ([]DbForeignKeyInfo, error) {
	query := `
		SELECT
			tc.constraint_name,
			tc.table_name,
	...
	fkMap := make(map[string]*DbForeignKeyInfo)
	...
	for rows.Next() {
		...
		if _, exists := fkMap[constraintName]; !exists {
			fkMap[constraintName] = &DbForeignKeyInfo{
