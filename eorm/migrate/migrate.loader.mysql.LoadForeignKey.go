...
func (m *MigratorLoaderMysql) LoadForeignKey(db *tenantDB.TenantDB) ([]DbForeignKeyInfo, error) {
	query := `
		SELECT
			rc.CONSTRAINT_NAME,
			kcu.TABLE_NAME,
	...
	temp := make(map[key]*DbForeignKeyInfo)
	...
	result := make([]DbForeignKeyInfo, 0, len(temp))
	for _, v := range temp {
