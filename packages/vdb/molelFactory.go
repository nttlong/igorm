...
func (m *modelFacoryType) resolveFixDefautValue(defaultValua string, fieldType reflect.Type) (*reflect.Value, error) {
	...
	item.once.Do(func() {
		...
		item.err = NewUnSupportError("default value not support: " + defaultValua)
	})
...
func (m *modelFacoryType) resolveDefautValue(modelType reflect.Type, defaultValua string, field reflect.StructField) (*reflect.Value, error) {
	...
	case "now()":
		if fieldType == reflect.TypeOf(time.Time{}) {
			...
			return nil, NewInvalidDefaultValue(field, modelType, defaultValua)
		}
	...
	default:
		{
			return nil, NewUnSupportError("default value not support: " + defaultValua)
		}
	}
}
...
func (m *modelFacoryType) getColsOfModelHasDefaultValue(typ reflect.Type) ([]migrate.ColumnDef, error) {
...
func NewFromModel[T any]() (*T, error) {
	typ := reflect.TypeFor[T]()
	val, err := modelFacory.CreateFromType(typ)
	if err != nil {
		return nil, err
	}
	return val.(*T), nil
