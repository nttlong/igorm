...
type AuthHandler struct { // <-- co hai cach de khai bao 1 handler
	vapi.Handler                  // khai bao 1 struc va embed vapi.handler
	Auth         *vapi.AuthClaims //<-- day la tuy chon, neu co 1 field vapi.AuthClaim, dieu nay co nghia la
...
func (m *Media) File(ctx struct {
	vapi.Handler `route:"method:get;uri:@/{FileName}"` //<-- tai sao la @/{FileName}?
	//
	//											^
	//				|--------------------------	|
	//										[Lay o cho nay]
...
func (a *Auth) Oauth(ctx *struct {
	vapi.Handler `route:"uri:/api/@/token"` //--> se phat sinh la uri la /api/oauth/token
}, data struct {
